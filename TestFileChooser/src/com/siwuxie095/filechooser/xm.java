...
public class xm {
     ...
     static HSSFSheet sheet = workbook.createSheet("软件杯");
	 ...
	 public static void getFileName(int order,String imgpath) 
     {
    	// String imgpath = "C:\\Users\\lenovo\\Desktop\\myimg"; // 路径
        File f = new File(imgpath);
         if (!f.exists()) //判断目录是否存在
         {
            System.out.println(imgpath + " not exists");
            return;
         }
         File fa[] = f.listFiles();
         for (int i = 0; i < fa.length; i++)  			//遍历输出目录名
         {
            ...
            else 
           {
            	...
            	//	System.out.println(fs.getAbsolutePath());			//输出完整的绝对路径
            		if(fs.getAbsolutePath().contains(".txt")) 
            		{
	            		File file = new File(fs.getAbsolutePath());					
	            						//读取
	            /*		while(readLine(fs.getAbsolutePath())==null) {
	            			String neirong[] =readLine(fs.getAbsolutePath());
	            		}*/
            	...
            	if(order==3&&fs.getAbsolutePath().contains("txt")) 
            	{
            		File file = new File(fs.getAbsolutePath());	
            		while(true) 
            		{
	...
	static void zong(String imgPath,String excelPath){
		getFileName(1,imgPath);
		getFileName(3,imgPath);
		getFileName(2,imgPath);							//最后
	...
	public static void main(String[] args) {
		zong("","C:\\Users\\lenovo\\Desktop\\肖振宇.xls");
